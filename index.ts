export const levenshtein = (src: string, target: string): number => {
    ...
    for (let i = 0; i < m; i++) { dMatrix[i] = new Array(n).fill(0) }
    for (let i = 1; i < m; i++) { dMatrix[i][0] = i }
    for (let j = 1; j < n; j++) { dMatrix[0][j] = j }
    ...
    for (let j = 1; j < n; j++) {
        for (let i = 1; i < m; i++) {
            ...
            ].reduce((prev: number, current: number) => prev < current ? prev : current)
        }
    }
